...
from turbo.models.base_model import BaseModel
...
class TurboOneCycleModel(BaseModel):

    """
    This class implements the Turbo model with one cycle
    """
    ...
    def __init__(self, opt):
        """Initialize Turbo one cycle model

        Parameters:
            opt (Option class)-- stores all the experiment flags; needs to be a subclass of BaseOptions
        """
        BaseModel.__init__(self, opt)
        self.loss_names = ['cycle_A', 'idt_A', 'idt_B', 'est_A', 'est_B',  'G_A', 'G_B', 'D_A', 'D_B']
        self.visual_names = ['real_A', 'est_A', 'fake_A', 'real_B', 'est_B', 'fake_B']
        ...
        if self.isTrain:  # define discriminators
            ...
            self.netD_B = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            D_parameters = list(self.netD_A.parameters()) + list(self.netD_B.parameters())

        if self.isTrain:
            if opt.lambda_identity > 0.0:  # only works when input and output images have the same number of channels
                assert(opt.input_nc == opt.output_nc)
            self.est_A_pool = ImagePool(opt.pool_size)  # create image buffer to store previously generated images
            self.est_B_pool = ImagePool(opt.pool_size)  # create image buffer to store previously generated images
    ...
    def forward(self):
        ...
        self.est_A = self.netG_B(self.est_B)  # G_B(G(A))
    ...
    def backward_D(self):
        ...
        for domain in ['A', 'B']:
            fake = getattr(self, f'est_{domain}')
            real = getattr(self, f'real_{domain}')
            net_D = getattr(self, f'netD_{domain}')
            fake = getattr(self, f'est_{domain}_pool').query(fake)
            loss_G = getattr(self, f'loss_G_{domain}')
            loss_D, update_loss = self.backward_D_basic(net_D, real, fake, loss_G)
            if update_loss:
                setattr(self, f'loss_D_{domain}', loss_D)
    ...
    def backward_G(self):
        ...
        for domain in ['A', 'B']:
            ...
            setattr(self, f'loss_G_{domain}', loss_G)

        # Forward cycle loss || G_B(G_A(A)) - A||
        self.loss_cycle_A = self.criterionCycle(self.est_A, self.real_A) * lambda_A
        # combined loss and calculate gradients
        self.total_loss_G += self.loss_cycle_A + self.loss_idt_A + self.loss_idt_B + self.loss_est_A + self.loss_est_B
