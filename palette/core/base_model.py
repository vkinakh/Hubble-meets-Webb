...
class BaseModel:
    def __init__(self, opt, phase_loader, val_loader, metrics, logger, writer):
        ...
        # best val score
        self.best_val_score = 1e10

    def train(self):
        while self.epoch <= self.opt['train']['n_epoch'] and self.iter <= self.opt['train']['n_iter']:
            self.epoch += 1
            if self.opt['distributed']:
                """sets the epoch for this sampler. When :attr:`shuffle=True`, this ensures all replicas use a 
                different random ordering for each epoch"""
            ...
            if self.epoch % self.opt['train']['val_epoch'] == 0:
                ...
                val_score = val_log['val/mae']
                if val_score < self.best_val_score:
                    self.best_val_score = val_score
                    self.save_everything(best=True)
    ...
    def load_network(self, network, network_label, strict=True):
        ...
        if isinstance(network, nn.DataParallel) or isinstance(network, nn.parallel.DistributedDataParallel):
            network = network.module
        res = network.load_state_dict(torch.load(model_path,
                                                 map_location=lambda storage, loc: Util.set_device(storage)),
                                      strict=strict)
        print(f'Loaded: {res}')
